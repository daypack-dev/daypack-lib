...
let flatten_to_seq_internal ~(modulo : int option) ~(of_int : int -> 'a)
  ...
  | `Range_inc (start, end_inc) -> (
      ...
      if start_int <= end_inc_int then
        OSeq.(start_int -- end_inc_int) |> Seq.map of_int
      else
        match modulo with
        | None -> raise (Invalid_argument "End is before start")
        | Some modulo ->
          if modulo <= 0 then raise (Invalid_argument "Modulo is <= 0")
          else
            OSeq.append OSeq.(start_int --^ modulo) OSeq.(0 -- end_inc_int)
            |> Seq.map of_int )
  | `Range_exc (start, end_exc) -> (
      let start_int = to_int start in
      let end_exc_int = to_int end_exc in
      if start_int <= end_exc_int then
        OSeq.(start_int --^ end_exc_int) |> Seq.map of_int
      else
        match modulo with
        | None -> raise (Invalid_argument "End is before start")
        | Some modulo ->
          if modulo <= 0 then raise (Invalid_argument "Modulo is <= 0")
            ...
            OSeq.append OSeq.(start_int --^ modulo) OSeq.(0 --^ end_exc_int)
            |> Seq.map of_int )

let flatten_to_seq ?(modulo : int option) ~(of_int : int -> 'a)
    ~(to_int : 'a -> int) (t : 'a t) : 'a Seq.t =
  flatten_to_seq_internal ~modulo ~of_int ~to_int t

let flatten_to_list ?(modulo : int option) ~(of_int : int -> 'a)
    ~(to_int : 'a -> int) (t : 'a t) : 'a list =
  flatten_to_seq_internal ~modulo ~of_int ~to_int t |> List.of_seq
    ...
    | Seq.Cons (x, rest) -> (
        ...
        | Some (start, end_inc) ->
          if b_is_next_of_a ~to_int end_inc x then
            ...
            Seq.Cons (`Range_inc (start, end_inc), aux to_int None rest) )
  in
  aux to_int None s
...
let range_seq_of_list (type a) ~(to_int : a -> int) (l : a list) : a t Seq.t =
  List.to_seq l |> range_seq_of_seq ~to_int

let range_list_of_list (type a) ~(to_int : a -> int) (l : a list) : a t list =
  List.to_seq l |> range_seq_of_seq ~to_int |> List.of_seq
...
let compress_list (type a) ~(to_int : a -> int) (l : a t list) : a t list =
  List.to_seq l |> compress_seq ~to_int |> List.of_seq
