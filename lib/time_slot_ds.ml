...
module Multi = struct
  ...
  let seq_of_unix_time_seq ?(skip_sort = false) (s : int64 Seq.t) : t Seq.t =
    ...
    aux None s
    |> fun s ->
    if skip_sort then s else normalize ~skip_filter:true ~skip_sort s
      ...
      | ( Seq.Cons ((mem_of_start, mem_of_end_exc), mem_of_rest),
        ...
        -> (
            if mem_of_end_exc < not_mem_of_start then
              ...
              | Some s1, None ->
                fun () -> Seq.Cons (s1, aux mem_of_rest not_mem_of)
              | None, Some s2 ->
    ...
    let rec aux time_slots1 time_slots2 : t Seq.t =
      ...
      | ( Seq.Cons ((start1, end_exc1), rest1),
        ...
        if end_exc1 < start2 then
          ...
          let s1 =
            if end_exc1 <= overlap_end_exc then rest1 else time_slots1
          in
          let s2 =
            if end_exc2 <= overlap_end_exc then rest2 else time_slots2
          in
