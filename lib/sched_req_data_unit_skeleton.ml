...
let shift_time ~offset (t : ('a, int64, Time_slot_ds.t) t) :
  ('a, int64, Time_slot_ds.t) t =
  match t with
  | Fixed { task_seg_related_data; start } ->
    Fixed { task_seg_related_data; start = start +^ offset }
  | Shift x ->
    Shift { x with time_slots = Time_slot_ds.shift_list ~offset x.time_slots }
  | Split_and_shift x ->
    Split_and_shift
      { x with time_slots = Time_slot_ds.shift_list ~offset x.time_slots }
  | Split_even x ->
    Split_even
      {
        x with
        time_slots = Time_slot_ds.shift_list ~offset x.time_slots;
        buckets = Time_slot_ds.shift_list ~offset x.buckets;
      }
  | Time_share x ->
    Time_share
      { x with time_slots = Time_slot_ds.shift_list ~offset x.time_slots }
...
let shift_time_list ~offset (ts : ('a, int64, Time_slot_ds.t) t list) :
  ('a, int64, Time_slot_ds.t) t list =
      ...
      Shift
        {
          ...
          incre = Misc_utils.int32_int32_to_int64 x.incre;
        }
    | `Split_and_shift x ->
      Split_and_shift
        {
          ...
          split_count =
            ( match x.split_count with
              | `Max_split x -> Max_split (Misc_utils.int32_int32_to_int64 x)
              | `Exact_split x ->
                Exact_split (Misc_utils.int32_int32_to_int64 x) );
          min_seg_size = Misc_utils.int32_int32_to_int64 x.min_seg_size;
          max_seg_size =
            Option.map Misc_utils.int32_int32_to_int64 x.max_seg_size;
        }
    | `Split_even x ->
      Split_even
        {
      ...
      Time_share
        {
          task_seg_related_data_list =
            List.map unpack_data x.task_seg_related_data_list;
          time_slots = List.map unpack_time_slot x.time_slots;
          interval_size = Misc_utils.int32_int32_to_int64 x.interval_size;
        }
    | `Push_toward x ->
      Push_toward
        {
