...
let add_hour_minute_to_time_pattern (e : hour_minute_expr)
    (pat : Time_pattern.t) : Time_pattern.t =
  { pat with hours = e.hour :: pat.hours; minutes = e.minute :: pat.minutes }
...
let paired_time_patterns_list_of_time_slots_expr (e : time_slots_expr) :
  ...
  | Hour_minutes_of_day_list { hour_minutes; days } ->
    let hm_start, hm_end_exc =
      paired_hour_minute_of_range_expr hour_minutes
    in
    days
    |> List.to_seq
