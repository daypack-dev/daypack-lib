...
module To_string = struct
  let debug_string_of_hour_minutes ({hour; minute; mode} : Time_expr_ast.hour_minute_expr) : string =
    Printf.sprintf "%d:%d%s" hour minute (match mode with
        | Hour_in_AM -> " am"
        | Hour_in_PM -> " pm"
        | Hour_in_24_hours -> ""
      )
    ...
    if 0 <= minute && minute < 60 then (
      ...
      | Hour_in_AM ->
        if 0 <= hour && hour < 12 then
          ...
          raise (Invalid_time_expr ("Invalid hour : " ^ (To_string.debug_string_of_hour_minutes e)))
      | Hour_in_PM ->
        if 0 <= hour && hour < 12 then
          { hour = hour + 12; minute }
        else
...
module Interpret_string = struct
  let parse lexbuf : Time_expr_ast.t = Time_expr_parser.parse Time_expr_lexer.read lexbuf
