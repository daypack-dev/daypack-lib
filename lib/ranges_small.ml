let normalize (type a) ?(skip_filter_invalid = false) ?(skip_filter_empty = false) ?(skip_sort = false)
  ...
  Ranges.normalize ~skip_filter_invalid ~skip_filter_empty ~skip_sort ~to_int64 ~of_int64 s

module Flatten = struct
  let flatten_internal (type a) ~(modulo : int option) ~(to_int : a -> int)
      ~(of_int : int -> a) (s : a Range.range Seq.t) : a Seq.t =
    let to_int64 = Misc_utils.convert_to_int_to_int64 to_int in
    let of_int64 = Misc_utils.convert_of_int_to_int64 of_int in
    match modulo with
    | None -> Ranges.Flatten.flatten ~to_int64 ~of_int64 s
    | Some modulo ->
  ...
  let flatten_list (type a) ?(modulo : int option) ~(to_int : a -> int)
    ...
    l |> List.to_seq |> flatten_internal ~modulo ~to_int ~of_int |> List.of_seq
end
...
module Of_seq = struct
  let range_seq_of_seq (type a) ?(skip_filter_invalid = false) ?(skip_filter_empty = false) ?(skip_sort = false)
    ...
    |> normalize ~skip_filter_invalid ~skip_filter_empty ~skip_sort ~to_int ~of_int

  let range_list_of_seq (type a) ?(skip_filter_invalid = false) ?(skip_filter_empty = false) ?(skip_sort = false)
      ~(to_int : a -> int) ~(of_int : int -> a) (s : a Seq.t) :
    a Range.range list =
    range_seq_of_seq ~skip_filter_invalid ~skip_filter_empty ~skip_sort ~to_int ~of_int s |> List.of_seq
end
...
module Of_list = struct
  let range_seq_of_list (type a) ?(skip_filter_invalid = false) ?(skip_filter_empty = false) ?(skip_sort = false)
    ...
    a Range.range Seq.t =
    List.to_seq l
    |> Of_seq.range_seq_of_seq ~skip_filter_invalid ~skip_filter_empty ~skip_sort ~to_int ~of_int
...
module Make (B : Range_small.B) : Ranges.S with type t := B.t = struct
  ...
  let normalize ?(skip_filter_invalid = false) ?(skip_filter_empty = false) ?(skip_sort = false)
    ...
    normalize ~skip_filter_invalid ~skip_filter_empty ~skip_sort ~to_int ~of_int s
