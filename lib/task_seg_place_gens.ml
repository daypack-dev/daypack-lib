...
let multi_task_segs_interleave ~interval_size
    ~(task_segs : Task_ds.task_seg list) (time_slots : Time_slot.t Seq.t) :
  ...
  | _ ->
    ...
    let time_slots_chunked =
      Time_slots.chunk ~chunk_size:interval_size ~drop_partial:true
        time_slots
    in
    let task_segs =
...
let single_task_seg_multi_even_splits ~incre ~(task_seg : Task_ds.task_seg)
  ...
  let possibly_usable_buckets =
    buckets
    |> List.map (fun bucket ->
        Time_slots.intersect (Seq.return bucket) usable_time_slots
        |> List.of_seq)
  in
