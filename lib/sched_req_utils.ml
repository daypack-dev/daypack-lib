let sched_req_template_matches_sched_req_data_unit
    (sched_req_template : Task.sched_req_template)
    (sched_req_data_unit : Sched_req.sched_req_data_unit) : bool =
  match (sched_req_template, sched_req_data_unit) with
  ...
  | ( Sched_req_data_unit_skeleton.Shift {task_seg_related_data_list = sizes1;
                                          time_slots = time_slots1; _ },
      Sched_req_data_unit_skeleton.Shift {task_seg_related_data_list = task_segs;
                                          time_slots = time_slots2; _ } )
  | ( Sched_req_data_unit_skeleton.Time_share {task_seg_related_data_list = sizes1;
                                               time_slots = time_slots1; _ },
      Sched_req_data_unit_skeleton.Time_share {task_seg_related_data_list = task_segs;
                                               time_slots = time_slots2; _ }) ->
  ...
  | ( Sched_req_data_unit_skeleton.Split_and_shift {task_seg_related_data = size1;
                                                    time_slots = time_slots1; _ },
      Sched_req_data_unit_skeleton.Split_and_shift {task_seg_related_data = (_id, size2);
                                                    time_slots = time_slots2; _ } )
    ->
  ...
  | ( Push_toward {task_seg_related_data = size1;
                   target = target1;
                   time_slots = time_slots1},
      Push_toward {task_seg_related_data = (_id, size2);
                   target = target2;
                   time_slots = time_slots2} ) ->
    target1 = target2 && size1 = size2 && Time_slot.equal time_slots1 time_slots2
  | _ -> false
...
let sched_req_template_matches_sched_req_record_data_unit
  ...
  match (sched_req_template, sched_req_record_data_unit) with
  | ( Sched_req_data_unit_skeleton.Fixed
        { task_seg_related_data = size1; start = start1 },
      Sched_req_data_unit_skeleton.Fixed
        { task_seg_related_data = _id, size2; start = start2 } ) ->
  ...
  | ( Sched_req_data_unit_skeleton.Split_even
    ...
    && Time_slot.equal time_slots1 time_slots2
    && Time_slot.equal buckets1 buckets2
  | ( Push_toward {task_seg_related_data =size1; target = target1; time_slots  = time_slots1},
      Push_toward {task_seg_related_data = (_id, size2); target = target2; time_slots = time_slots2} ) ->
    target1 = target2 && size1 = size2 && Time_slot.equal time_slots1 time_slots2
  | _ -> false
...
let sched_req_template_matches_sched_req_record_data
  ...
  List.for_all
    (fun sched_req_record_data ->
       sched_req_template_matches_sched_req_record_data_unit sched_req_template
