...
module Print = struct
  let debug_string_sched_req_data ?(indent_level = 0) ?(buffer = Buffer.create 4096) req_data =
    begin
    match req_data with
    | Sched_req_data_skeleton.Fixed { task_seg_related_data = id, len; start }
      ...
      Debug_print.bprintf ~indent_level:(indent_level + 2) buffer "%s : %Ld\n"
        ...
        len;
      Debug_print.bprintf ~indent_level:(indent_level + 1) buffer "start :\n";
      Debug_print.bprintf ~indent_level:(indent_level + 2) buffer "%Ld\n" start
      ...
      List.iter
        (fun (id, len) ->
           Debug_print.bprintf ~indent_level:(indent_level + 2) buffer "%s : %Ld\n"
             ...
             len)
        task_seg_allocs;
      Debug_print.bprintf ~indent_level:(indent_level + 1) buffer "time slots :\n";
      List.iter
        (fun (start, end_exc) ->
           Debug_print.bprintf ~indent_level:(indent_level + 2) buffer "[%Ld, %Ld)\n"
             start end_exc)
        time_slots
    ...
    | Push_to (side, (id, len), time_slots) ->
      Debug_print.bprintf ~indent_level buffer "push to %s:\n"
  ...
  let debug_string_sched_req ?(indent_level = 0) ?(buffer = Buffer.create 4096) (id, req_data) =
    Debug_print.bprintf ~indent_level buffer "schedule request id : %Ld\n" id;
    debug_string_sched_req_data ~indent_level:(indent_level + 1) ~buffer req_data |> ignore;
    Buffer.contents buffer
