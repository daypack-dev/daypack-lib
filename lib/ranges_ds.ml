  module Normalize = struct
    ...
    let normalize (type a) ?(skip_filter = false) ?(skip_sort = false)
      ...
      |> Time_slots_ds.normalize ~skip_filter ~skip_sort
  ...
  module Compress = struct
    let compress_seq_big (type a) ~(to_int64 : a -> int64) (s : a Range_ds.t Seq.t) :
      a Range_ds.t Seq.t =
      let rec aux (to_int64 : a -> int64) (acc : a Range_ds.t option) (s : a Range_ds.t Seq.t) :
        a Range_ds.t Seq.t =
        match s () with
        | Seq.Nil -> (
    ...
    let compress_list_big (type a) ~(to_int64 : a -> int64) (l : a Range_ds.t list) :
      a Range_ds.t list =
      List.to_seq l |> compress_seq_big ~to_int64 |> List.of_seq

    let compress_seq (type a) ~(to_int : a -> int) (s : a Range_ds.t Seq.t) : a Range_ds.t Seq.t =
      let to_int64 = Misc_utils.convert_to_int_to_int64 to_int in
      compress_seq_big ~to_int64 s

    let compress_list (type a) ~(to_int : a -> int) (l : a Range_ds.t list) : a Range_ds.t list =
      let to_int64 = Misc_utils.convert_to_int_to_int64 to_int in
