...
let debug_single_task_seg_shift () =
  ...
  let time_segs = [ (0L, 10L); (20L, 24L) ] in
  Printf.printf "incre : %Ld\n" incre;
  Printf.printf "cur_pos : %Ld\n" cur_pos;
  Printf.printf "task_seg_size : %Ld\n" task_seg_size;
  List.iteri
    (fun i (start, end_exc) ->
       Printf.printf "time_seg #%d : (%Ld, %Ld)\n" i start end_exc)
    time_segs;
  print_newline ();
  let time_segs = List.to_seq time_segs in
  Task_seg_place_gens.single_task_seg_shift ~incre ~cur_pos ~task_seg time_segs
...
let debug_multi_task_segs_shift () =
  ...
  print_newline ();
  let time_segs = List.to_seq time_segs in
  Task_seg_place_gens.multi_task_segs_shift ~incre ~task_segs time_segs
  |> Seq.iter (fun pos_s ->
...
let debug_single_task_seg_multi_splits_exact_shift () =
  ...
  let task_seg = (task_seg_id, task_seg_size) in
  let time_segs = [ (0L, 10L); (11L, 15L) ] in
  Printf.printf "min_seg_size : %Ld\n" min_seg_size;
  Printf.printf "split count : %Ld\n" split_count;
  Printf.printf "task_seg_size : %Ld\n" task_seg_size;
  List.iteri
  ...
  Task_seg_place_gens.single_task_seg_multi_splits_exact_shift ~min_seg_size
    ~max_seg_size ~split_count ~incre ~task_seg time_segs
  |> Seq.iter (fun splits ->
      Printf.printf "splits :\n";
      List.iter
        (fun (id, start, end_exc) ->
...
let debug_single_task_seg_multi_even_splits () =
  ...
  in
  let usable_time_segs = [ (0L, 500L) ] in
  let usable_time_segs = List.to_seq usable_time_segs in
  let s =
    Task_seg_place_gens.single_task_seg_multi_even_splits ~incre:5L ~task_seg
      ~buckets ~usable_time_segs
  in
  s
  |> Seq.iter (fun splits ->
      Printf.printf "splits :\n";
...
let debug_slice_time_segs_start () =
  print_endline "Debug print for Time_segs.slice start";
  let time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs = List.to_seq time_segs in
  Time_segs.Slice.slice ~start:12L time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_slice_time_segs_end_exc () =
  print_endline "Debug print for Time_segs.slice end_exc";
  let time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs = List.to_seq time_segs in
  Time_segs.Slice.slice ~end_exc:12L time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_slice_time_segs_start_rev () =
  print_endline "Debug print for Time_segs.slice_rev start";
  let time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs = time_segs |> List.rev |> List.to_seq in
  Time_segs.Slice.slice_rev ~start:12L time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_slice_time_segs_end_exc_rev () =
  print_endline "Debug print for Time_segs.slice_rev end_exc";
  let time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs = time_segs |> List.rev |> List.to_seq in
  Time_segs.Slice.slice_rev ~end_exc:12L time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_normalize_time_segs () =
  print_endline "Debug print for Time_segs.normalize";
  let time_segs =
    [ (0L, 10L); (10L, 11L); (11L, 20L); (22L, 25L); (20L, 22L); (25L, 30L) ]
  in
  let time_segs = List.to_seq time_segs in
  Time_segs.Normalize.normalize time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_invert_time_segs () =
  print_endline "Debug print for Time_segs.invert";
  let time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs = List.to_seq time_segs in
  Time_segs.invert ~start:0L ~end_exc:22L time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_relative_complement_time_segs () =
  print_endline "Debug print for Time_segs.relative_complement";
  let mem_of_time_segs = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let not_mem_of_time_segs = [ (0L, 5L); (6L, 15L); (25L, 30L) ] in
  let mem_of_time_segs = List.to_seq mem_of_time_segs in
  let not_mem_of_time_segs = List.to_seq not_mem_of_time_segs in
  Time_segs.relative_complement ~not_mem_of:not_mem_of_time_segs
    mem_of_time_segs
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_intersect_time_segs () =
  print_endline "Debug print for Time_segs.intersect";
  let time_segs1 = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs2 = [ (0L, 1L); (2L, 3L); (10L, 20L); (25L, 30L) ] in
  let time_segs1 = List.to_seq time_segs1 in
  let time_segs2 = List.to_seq time_segs2 in
  Time_segs.intersect time_segs1 time_segs2
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)

let debug_union_time_segs () =
  print_endline "Debug print for Time_segs.union";
  let time_segs1 = [ (0L, 10L); (11L, 20L); (25L, 30L) ] in
  let time_segs2 = [ (0L, 1L); (2L, 3L); (10L, 20L); (25L, 30L) ] in
  let time_segs1 = List.to_seq time_segs1 in
  let time_segs2 = List.to_seq time_segs2 in
  Time_segs.Union.union time_segs1 time_segs2
  |> Seq.iter (fun (start, end_exc) ->
      Printf.printf "  [%Ld, %Ld)\n" start end_exc)
...
let debug_sched_agenda_range () =
  ...
  |> Sched.Agenda.To_seq.task_seg_place_all ~start:9L ~end_exc:40L
    ...
    ~include_task_seg_place_ending_within_time_seg:true
...
let debug_sched_usage_simulation () =
   ...
   *       ~include_sched_reqs_partially_within_time_seg:true
   *       ~up_to_sched_req_id_inc:None sched_ver_history
   *   with
   *   | Ok () ->
   *     print_endline "Scheduled successfully";
...
let debug_time_pattern_matching_time_segs () =
  print_endline "Debug print for Time_pattern.matching_time_segs";
  let date_time =
  ...
  Daypack_lib.Time_pattern.Print.debug_print_time_pattern pattern;
  let s =
    Daypack_lib.Time_pattern.Single_pattern.matching_time_segs
...
let debug_time_range_pattern_matching_time_segs () =
  print_endline
    "Debug print for Time_pattern.matching_time_segs_time_range_pattern";
  let date_time =
    Time.Current.cur_date_time ~tz_offset_s_of_date_time:None |> Result.get_ok
  in
  let start = Time.unix_second_of_date_time date_time |> Result.get_ok in
  ...
  let s =
    Daypack_lib.Time_pattern.Range_pattern.matching_time_segs
      (Time_segs { search_using_tz_offset_s = None; time_segs })
      pattern
    |> Result.get_ok
  in
...
let debug_time_profile_matching_time_segs_of_periods () =
  print_endline "Debug print for Time_profile.matching_time_segs_of_periods";
  ...
  in
  let s = Time_profile.matching_time_segs_of_periods ~start ~end_exc periods in
  s
  |> OSeq.take 10
  |> OSeq.iteri (fun i (start, end_exc) ->
      Printf.printf "iter : %d\n" i;
 ...
 *   let time_segs = [ (\* (0L, 36_347_213L) *\) ] in
 *   Daypack_lib.Time_pattern.Print.debug_print_pattern pattern;
 *   for i = 0 to 10 do
 *     Printf.printf "iter : %d\n" i;
 *     match !time with
 ...
 *       time :=
 *         Daypack_lib.Time_pattern.next_match_int64 ~time_segs ~normalize_dir
 *           ~search_years_ahead pattern x
 *     | None -> print_endline "nothing"
 *   done *)
...
(* let () =
 *   debug_slice_time_segs_start ();
 *   print_newline () *)

(* let () =
 *   debug_slice_time_segs_end_exc ();
 *   print_newline () *)

(* let () =
 *   debug_slice_time_segs_start_rev ();
 *   print_newline () *)

(* let () =
 *   debug_slice_time_segs_end_exc_rev ();
 *   print_newline () *)

(* let () =
 *   debug_normalize_time_segs ();
 *   print_newline () *)

(* let () =
 *   debug_invert_time_segs ();
 *   print_newline () *)

(* let () =
 *   debug_relative_complement_time_segs ();
 *   print_newline () *)

(* let () =
 *   debug_intersect_time_segs ();
 *   print_newline () *)

(* let () =
 *   debug_union_time_segs ();
 *   print_newline () *)
...
(* let () =
 *   debug_time_pattern_matching_time_segs ();
 *   print_newline () *)

let () =
  debug_time_range_pattern_matching_time_segs ();
  print_newline ()
...
(* let () =
 *   debug_time_profile_matching_time_segs_of_periods ();
 *   print_newline () *)
