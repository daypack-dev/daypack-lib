...
let qc_normalize_idempotent_wrt_normalized_time_slots =
  QCheck.Test.make ~count:10_000
    ...
    sorted_time_slots_with_gaps (fun l ->
        l |> List.to_seq |> Daypack_lib.Time_slot.normalize |> List.of_seq = l)
...
let qc_relatvie_complement_result_disjoint_from_not_mem_of =
  QCheck.Test.make ~count:10_000
    ...
    (fun (mem_of, not_mem_of) ->
       let res =
         Daypack_lib.Time_slot.relative_complement ~mem_of:(List.to_seq mem_of)
...
let qc_intersect_with_self =
  QCheck.Test.make ~count:10_000 ~name:"qc_intersect_with_self"
    sorted_time_slots_maybe_gaps (fun l ->
        ...
        let res = Daypack_lib.Time_slot.intersect s s |> List.of_seq in
        l = res)
...
let qc_intersect_commutative =
  QCheck.Test.make ~count:10_000 ~name:"qc_intersect_commutative"
    ...
    (fun (l1, l2) ->
       ...
       let inter1 = Daypack_lib.Time_slot.intersect s1 s2 |> List.of_seq in
       let inter2 = Daypack_lib.Time_slot.intersect s2 s1 |> List.of_seq in
       inter1 = inter2)
...
let qc_intersect_associative =
  QCheck.Test.make ~count:10_000 ~name:"qc_intersect_associative"
    ...
    (fun (l1, l2, l3) ->
       ...
       let inter1 =
         Daypack_lib.Time_slot.(intersect (intersect s1 s2) s3) |> List.of_seq
       in
       let inter2 =
         Daypack_lib.Time_slot.(intersect s1 (intersect s2 s3)) |> List.of_seq
       in
       inter1 = inter2)
...
let qc_union_with_self =
  QCheck.Test.make ~count:10_000 ~name:"qc_union_with_self"
    sorted_time_slots_with_gaps (fun l ->
        let s = l |> List.to_seq in
        let res = Daypack_lib.Time_slot.union s s |> List.of_seq in
        l = res)
...
let qc_union_commutative =
  QCheck.Test.make ~count:10_000 ~name:"qc_union_commutative"
    ...
    (fun (l1, l2) ->
       let s1 = l1 |> List.to_seq in
       let s2 = l2 |> List.to_seq in
       let inter1 = Daypack_lib.Time_slot.union s1 s2 |> List.of_seq in
       let inter2 = Daypack_lib.Time_slot.union s2 s1 |> List.of_seq in
       inter1 = inter2)
...
let qc_union_associative =
  QCheck.Test.make ~count:10_000 ~name:"qc_union_associative"
    ...
    (fun (l1, l2, l3) ->
       ...
       let s3 = l3 |> List.to_seq in
       let res1 =
         Daypack_lib.Time_slot.(union (union s1 s2) s3) |> List.of_seq
       in
       let res2 =
         Daypack_lib.Time_slot.(union s1 (union s2 s3)) |> List.of_seq
       in
       res1 = res2)
...
let qc_intersect_union_distributive1 =
  QCheck.Test.make ~count:10_000 ~name:"qc_intersect_union_distributive1"
    ...
    (fun (l1, l2, l3) ->
       ...
       let s3 = l3 |> List.to_seq in
       let res1 =
         Daypack_lib.Time_slot.(union s1 (intersect s2 s3)) |> List.of_seq
       in
       let res2 =
         Daypack_lib.Time_slot.(intersect (union s1 s2) (union s1 s3))
         |> List.of_seq
       in
       res1 = res2)
...
let qc_intersect_union_distributive2 =
  QCheck.Test.make ~count:10_000 ~name:"qc_intersect_union_distributive2"
    ...
    (fun (l1, l2, l3) ->
       ...
       let s3 = l3 |> List.to_seq in
       let res1 =
         Daypack_lib.Time_slot.(intersect s1 (union s2 s3)) |> List.of_seq
       in
       let res2 =
         Daypack_lib.Time_slot.(union (intersect s1 s2) (intersect s1 s3))
         |> List.of_seq
       in
