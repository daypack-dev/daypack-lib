...
let arith_seq_gen =
  ...
  map3
    (fun start offset diff ->
       Daypack_lib.Task_ds.{ start; end_exc = Int64.add start offset; diff })
...
let progress_gen =
  let open QCheck.Gen in
  map (fun chunks -> Daypack_lib.Task_ds.{ chunks = Daypack_lib.Int64_int64_set.Deserialize.unpack chunks }) tiny_sorted_time_slots_gen
