...
module Qc = struct
    ...
    QCheck.Test.make ~count:10_000 ~name:"single_task_seg_shift_consistent"
      QCheck.(
        triple nz_pos_int64 pos_int64
          (pair (pair task_seg_id small_pos_int64) tiny_sorted_time_segs))
      (fun (incre, cur_pos, ((task_seg_id, task_seg_size), time_segs)) ->
  ...
  let single_task_seg_shift_rev_consistent =
    QCheck.Test.make ~count:10_000 ~name:"single_task_seg_shift_rev_consistent"
      QCheck.(
        triple nz_pos_int64 pos_int64
          (pair (pair task_seg_id small_pos_int64) tiny_sorted_time_segs))
      (fun (incre, cur_end_pos_exc, ((task_seg_id, task_seg_size), time_segs)) ->
         let time_segs = List.to_seq time_segs in
  ...
  let multi_task_segs_shift_uniq =
    QCheck.Test.make ~count:1000 ~name:"multi_task_segs_shift_uniq"
      QCheck.(triple nz_pos_int64 task_segs tiny_sorted_time_segs)
      (fun (incre, task_segs, time_segs) ->
         let time_segs = List.to_seq time_segs in
         let l =
    ...
    QCheck.Test.make ~count:1000
      ...
      QCheck.(
        quad nz_pos_int64
          ...
          small_pos_int64
          (pair task_seg tiny_sorted_time_segs))
      (fun ( min_seg_size,
