...
type ('task_seg_related_data, 'time_seg) shift = {
  task_seg_related_data_list : 'task_seg_related_data list;
  time_segs : 'time_seg list;
  incre : int64;
}
...
type ('task_seg_related_data, 'time_seg) split_and_shift = {
  task_seg_related_data : 'task_seg_related_data;
  time_segs : 'time_seg list;
  incre : int64;
...
type ('task_seg_related_data, 'time_seg) split_even = {
  task_seg_related_data : 'task_seg_related_data;
  time_segs : 'time_seg list;
  buckets : 'time_seg list;
  incre : int64;
}

type ('task_seg_related_data, 'time_seg) time_share = {
  task_seg_related_data_list : 'task_seg_related_data list;
  time_segs : 'time_seg list;
  interval_size : int64;
}

type ('task_seg_related_data, 'time, 'time_seg) push_toward = {
  task_seg_related_data : 'task_seg_related_data;
  target : 'time;
  time_segs : 'time_seg list;
  incre : int64;
...
type ('task_seg_related_data, 'time, 'time_seg) t =
  | Fixed of ('task_seg_related_data, 'time) fixed
  | Shift of ('task_seg_related_data, 'time_seg) shift
  | Split_and_shift of ('task_seg_related_data, 'time_seg) split_and_shift
  | Split_even of ('task_seg_related_data, 'time_seg) split_even
  | Time_share of ('task_seg_related_data, 'time_seg) time_share
  | Push_toward of ('task_seg_related_data, 'time, 'time_seg) push_toward

let shift_time ~offset (t : ('a, int64, Time_seg.t) t) :
  ('a, int64, Time_seg.t) t =
  match t with
  | Fixed { task_seg_related_data; start } ->
    Fixed { task_seg_related_data; start = start +^ offset }
  | Shift x ->
    Shift { x with time_segs = Time_segs.shift_list ~offset x.time_segs }
  | Split_and_shift x ->
    Split_and_shift
      { x with time_segs = Time_segs.shift_list ~offset x.time_segs }
  | Split_even x ->
    Split_even
      {
        x with
        time_segs = Time_segs.shift_list ~offset x.time_segs;
        buckets = Time_segs.shift_list ~offset x.buckets;
      }
  | Time_share x ->
    Time_share { x with time_segs = Time_segs.shift_list ~offset x.time_segs }
  | Push_toward x ->
    Push_toward
      {
        x with
...
let shift_time_list ~offset (ts : ('a, int64, Time_seg.t) t list) :
  ('a, int64, Time_seg.t) t list =
  List.map (shift_time ~offset) ts

let map (type a b c d e f) ~(f_data : a -> d) ~(f_time : b -> e)
    ~(f_time_seg : c -> f) (t : (a, b, c) t) : (d, e, f) t =
  match t with
  | Fixed { task_seg_related_data; start } ->
    Fixed
      {
    ...
    Shift
      {
        ...
        time_segs = List.map f_time_seg x.time_segs;
      }
  | Split_and_shift x ->
    Split_and_shift
      {
    ...
    Split_even
      {
        ...
        task_seg_related_data = f_data x.task_seg_related_data;
        time_segs = List.map f_time_seg x.time_segs;
        buckets = List.map f_time_seg x.buckets;
      }
  | Time_share x ->
    Time_share
      {
...
let map_list ~f_data ~f_time ~f_time_seg ts =
  List.map (map ~f_data ~f_time ~f_time_seg) ts

module Check = struct
  let check (type a b c) ~(f_data : a -> bool) ~(f_time : b -> bool)
      ~(f_time_seg : c -> bool) (t : (a, b, c) t) : bool =
    match t with
    | Fixed { task_seg_related_data; start } ->
      f_data task_seg_related_data && f_time start
    | Shift { task_seg_related_data_list; time_segs; incre } ->
      List.for_all f_data task_seg_related_data_list
      && List.for_all f_time_seg time_segs
      && incre > 0L
    | Split_and_shift
        {
    ...
    | Split_even { task_seg_related_data; time_segs; buckets; incre } ->
      f_data task_seg_related_data
      && List.for_all f_time_seg time_segs
      && List.for_all f_time_seg buckets
      && incre > 0L
    | Time_share { task_seg_related_data_list; time_segs; interval_size } ->
      List.for_all f_data task_seg_related_data_list
      && List.for_all f_time_seg time_segs
      && interval_size > 0L
    | Push_toward { task_seg_related_data; target; time_segs; incre } ->
      f_data task_seg_related_data
      && f_time target
      && List.for_all f_time_seg time_segs
      && incre > 0L
...
module To_string = struct
  let debug_string_of_sched_req_data_unit_skeleton (type a b c)
      ...
      ~(string_of_time_seg : c -> string) (t : (a, b, c) t) =
    ( match t with
      | Fixed { task_seg_related_data; start } ->
      ...
      `Shift
        {
          ...
          time_segs = List.map pack_time_seg x.time_segs;
        }
    | Split_and_shift x ->
      `Split_and_shift
        {
      ...
      `Split_even
        {
          task_seg_related_data = pack_data x.task_seg_related_data;
          time_segs = List.map pack_time_seg x.time_segs;
          buckets = List.map pack_time_seg x.buckets;
          incre = Misc_utils.int32_int32_of_int64 x.incre;
        }
    | Time_share x ->
      `Time_share
      ...
      Shift
        {
          ...
          time_segs = List.map unpack_time_seg x.time_segs;
      ...
      Split_even
        {
          task_seg_related_data = unpack_data x.task_seg_related_data;
          time_segs = List.map unpack_time_seg x.time_segs;
          buckets = List.map unpack_time_seg x.buckets;
          incre = Misc_utils.int64_of_int32_int32 x.incre;
        }
    | `Time_share x ->
      Time_share
