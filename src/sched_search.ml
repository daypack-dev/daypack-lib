...
let brute_force_single ~start ~end_exc ~(base : Sched.sched)
  ...
  Sched.sched Seq.t =
  let free_time_segs =
    Sched.Agenda.Time_seg.get_free_time_segs ~start ~end_exc base
  in
  ...
  in
  let get_usable_time_segs (task_segs : Task.task_seg list) time_segs =
    let up_to_task_seg_place_count_inc =
      match task_segs with
      | [] -> 0
      | l ->
        l
        |> List.map get_parallelizability
        |> List.sort_uniq compare
        |> List.hd
    in
    let time_seg_candidates =
      Time_segs.Union.union free_time_segs
        (Sched.Agenda.Time_seg
         .get_free_or_occupied_time_segs_up_to_task_seg_place_count ~start
           ~end_exc ~up_to_task_seg_place_count_inc base)
    in
    time_segs
    |> Time_segs.Normalize.normalize_list_in_seq_out
    |> Time_segs.intersect time_seg_candidates
  in
  Seq.flat_map
    (fun sched_req_record_data ->
       ...
       | Sched_req_data_unit_skeleton.Fixed
         ...
         let usable_time_segs =
           get_usable_time_segs [ task_seg ] [ (start, start +^ size) ]
         in
         Task_seg_place_gens.multi_task_segs_shift ~incre:1L
           ~task_segs:[ task_seg ] usable_time_segs
         |> OSeq.map (fun places ->
             base |> Sched.Agenda.Add.add_task_seg_place_list places)
       | Shift x ->
         let usable_time_segs =
           get_usable_time_segs x.task_seg_related_data_list x.time_segs
         in
         Task_seg_place_gens.multi_task_segs_shift ~incre:x.incre
           ~task_segs:x.task_seg_related_data_list usable_time_segs
         |> OSeq.map (fun places ->
             base |> Sched.Agenda.Add.add_task_seg_place_list places)
       | Split_and_shift x ->
         let usable_time_segs =
           get_usable_time_segs [ x.task_seg_related_data ] x.time_segs
         in
       ...
       | Push_toward x ->
         ...
         let s1 =
           Task_seg_place_gens.single_task_seg_shift ~cur_pos:x.target
             ~incre:x.incre ~task_seg:x.task_seg_related_data usable_time_segs
           |> OSeq.take 1
         in
         let s2 =
           Task_seg_place_gens.single_task_seg_shift_rev
             ~cur_end_pos_exc:x.target ~incre:x.incre
             ~task_seg:x.task_seg_related_data usable_time_segs
           |> OSeq.take 1
         in
         let s =
           OSeq.sorted_merge
...
let backtracking_search_pending ~start ~end_exc
    ...
    ~include_sched_reqs_ending_within_time_seg ~up_to_sched_req_id_inc ~base :
  Sched.sched Seq.t =
  let sched_req_records, base =
    Sched.Sched_req.Allocate_task_segs.allocate_task_segs_for_pending_sched_reqs
      ~start ~end_exc ~include_sched_reqs_starting_within_time_seg
      ~include_sched_reqs_ending_within_time_seg ~up_to_sched_req_id_inc base
  in
