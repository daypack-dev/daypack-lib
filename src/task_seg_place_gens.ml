      ...
      if end_exc -^ start >= task_seg_size then
        ...
        Seq.Cons
          ( (task_seg_id, end_exc -^ task_seg_size, end_exc),
            fun () -> aux incre (end_exc -^ incre) task_seg time_segs )
      else
        (* not big enough, move to next time slot *)
        aux incre cur_end_pos_exc task_seg slots
  in
  assert (incre > 0L);
  let time_segs = time_segs |> List.of_seq |> List.rev |> List.to_seq in
  fun () -> aux incre cur_end_pos_exc task_seg time_segs

let multi_task_segs_shift ~incre ~(task_segs : Task.task_seg list)
    (time_segs : Time_seg.t Seq.t) : Task.task_seg_place list Seq.t =
  assert (incre > 0L);
      ...
      (fun places_seq task_seg ->
         Seq.flat_map
           (fun places ->
              match places with
              | [] ->
                single_task_seg_shift ~incre ~cur_pos:0L ~task_seg time_segs
                |> Seq.map (fun x -> [ x ])
              | (last_id, last_start, last_end_exc) :: pos_s ->
                let time_segs =
                  Time_segs.Slice.slice ~start:last_end_exc time_segs
                in
...
let single_task_seg_multi_splits_exact_shift ~min_seg_size ~max_seg_size
  ...
  |> Seq.flat_map (fun task_segs ->
      multi_task_segs_shift ~incre ~task_segs time_segs)
...
let single_task_seg_multi_even_splits ~incre ~(task_seg : Task.task_seg)
  ...
  let possibly_usable_buckets =
    buckets
    |> List.map (fun bucket ->
        Time_segs.intersect (Seq.return bucket) usable_time_segs
        |> List.of_seq)
  in
