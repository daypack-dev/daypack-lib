...
and sched_req_data_unit =
  (Task.task_seg_alloc_req, int64, Time_segment.t) Sched_req_data_unit_skeleton.t
...
and sched_req_record_data_unit =
  (Task.task_seg, int64, Time_segment.t) Sched_req_data_unit_skeleton.t
...
let flexibility_score_of_sched_req_record
  ...
  | Shift x ->
    ...
    let time_slot_sum_len =
      Time_segments.Sum.sum_length_list x.time_slots |> Int64.to_float
    in
  ...
  | Split_even x ->
    let _, size = x.task_seg_related_data in
    let time_slot_sum_len =
      Time_segments.intersect
        (x.time_slots |> List.to_seq)
        (x.buckets |> List.to_seq)
      |> List.of_seq
      |> Time_segments.Sum.sum_length_list
      |> Int64.to_float
    in
    1. -. (Int64.to_float size /. time_slot_sum_len)
  | Time_share x ->
...
let start_and_end_exc_bound_of_sched_req_or_record
    ((_id, req_record_data_unit_list) :
       sched_req_id
       * ('a, int64, Time_segment.t) Sched_req_data_unit_skeleton.t list) :
  (int64 * int64) option =
  List.fold_left
...
let sched_req_or_record_before_time (x : int64)
    (sched_req_or_record :
       sched_req_id
       * ('a, int64, Time_segment.t) Sched_req_data_unit_skeleton.t list) : bool =
    ...
    List.for_all
      (fun x ->
         Sched_req_data_unit_skeleton.Check.check
           ...
           ~f_time_slot:Time_segment.Check.is_valid x)
      data
...
module Serialize = struct
  ...
  and pack_sched_req_data_unit (sched_req_data_unit : sched_req_data_unit) :
    ...
    Sched_req_data_unit_skeleton.Serialize.pack
      ...
      ~pack_time_slot:Time_segment.Serialize.pack_time_slot sched_req_data_unit
  ...
  and pack_sched_req_record_data_unit
    ...
    Sched_req_data_unit_skeleton.Serialize.pack
      ...
      ~pack_time:Misc_utils.int32_int32_of_int64
      ~pack_time_slot:Time_segment.Serialize.pack_time_slot sched_req_record_data
end
    ...
    .debug_string_of_sched_req_data_unit_skeleton ~indent_level ~buffer
      ~string_of_data:(fun (id, len) ->
          Printf.sprintf "task_id : %s, len : %Ld\n"
            ...
            len)
      ~string_of_time:Int64.to_string ~string_of_time_slot:Time_segment.to_string
      req_data
