...
let task_inst_arg = Arg.(value & flag & info [ "task-place" ])

let task_seg_arg = Arg.(value & flag & info [ "task-seg" ])

let run (complete_task : bool) (complete_task_inst : bool) (complete_task_seg : bool) : unit =
  ...
  | Ok context ->
    ...
    ( if complete_task then
        ...
        match Daypack_lib.Sched.Task.Find.find_task_any_with_status_opt task_id hd |> Option.get with
        ...
        | (data, `Uncompleted) ->
          if
            Dialog.ask_yn ~indent_level:0
              ~prompt:(Printf.sprintf "Really mark task \"%s\" as completed?" data.name) = `Yes
          then (
    ...
    ( if complete_task_seg then
        let task_seg_id = Dialog.ask_task_seg_id ~indent_level:0 ~exists_in_sched:(Some hd) in
        match Daypack_lib.Sched.Task_seg.Find.find_task_seg_any_with_status_opt task_seg_id hd |> Option.get with
        | (_, `Completed) -> print_endline "Task segment is marked as completed already"
        ...
        | (_, `Uncompleted) ->
          ...
          let task_data = Daypack_lib.Sched.Task.Find.find_task_any_opt task_id hd |> Option.get in
          if
            Dialog.ask_yn ~indent_level:0
...
let cmd = (Term.(const run $ task_arg $ task_inst_arg $ task_seg_arg), Term.info "complete")
