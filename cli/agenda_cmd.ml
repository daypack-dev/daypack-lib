...
let display_place ~start sched ((task_seg_id, place_start, place_end_exc) : Daypack_lib.Task_ds.task_seg_place) : unit =
  let start_str =
  ...
  in
  let time_str =
    Printf.sprintf "| %s - %s " start_str end_exc_str
  in
  let time_str_space =
    String.make (String.length time_str) ' '
  in
  Printf.printf "%s| %s \n" time_str task.name;
  Printf.printf "%s| ID: %s\n" time_str_space task_seg_id_str;
  Printf.printf "%s| From start: %s\n" time_str_space time_from_start_str

let display_places ~title ~show_all ~start ~end_exc ~max_count (sched : Daypack_lib.Sched.sched) : unit =
  let places =
...
let display_places_active_or_soon ~show_all ~cur_time sched : unit =
  ...
  in
  display_places ~title ~show_all ~start:cur_time ~end_exc ~max_count:Config.agenda_display_task_seg_place_soon_max_count
    sched
...
let display_places_close ~show_all ~cur_time sched : unit =
  ...
  in
  let end_exc =
    Int64.mul (Int64.of_int Config.agenda_search_day_count_close) Daypack_lib.Time.day_to_second_multiplier
    |> Int64.add cur_time
  in
  let title =
    Printf.sprintf
      "Next batch starting within next %d days%s:" Config.agenda_search_day_count_close
      (if show_all then "" else Printf.sprintf " (displaying up to %d task segments)" Config.agenda_display_task_seg_place_close_max_count);
  in
  display_places ~title ~show_all ~start ~end_exc ~max_count:Config.agenda_display_task_seg_place_close_max_count
    sched
...
let display_places_far ~show_all ~cur_time sched : unit =
  ...
  in
  let end_exc =
    Int64.mul (Int64.of_int Config.agenda_search_day_count_far) Daypack_lib.Time.day_to_second_multiplier
    |> Int64.add cur_time
  in
  let title =
    Printf.sprintf
      "Next batch starting within next %d days%s:" Config.agenda_search_day_count_far
      (if show_all then "" else Printf.sprintf " (displaying up to %d task segments)" Config.agenda_display_task_seg_place_far_max_count);
  in
  display_places ~title ~show_all ~start ~end_exc ~max_count:Config.agenda_display_task_seg_place_far_max_count
    sched
