...
let display_overdue_task_segs (context : Context.t) : unit =
  ...
  let overdue_task_seg_places =
    Daypack_lib.Sched.Overdue.get_overdue_task_seg_places ~deadline:(Daypack_lib.Time.Current.cur_unix_time ())
      hd
    |> List.of_seq
  in
  ...
  if count = 0 then print_endline "  - No overdue task segments"
    ...
    List.iter
      (fun (task_seg_id, place_start, place_end_exc) ->
         ...
         let task_data = Daypack_lib.Sched.Task.Find.find_task_uncompleted_opt
             task_id hd |> Option.get
         in
         let start_str =
